...
export class Scene {
    ...
    public NativeClass : Map<string, ArkClass> = new Map(); //用来记录原生类，如string、array、map等
    constructor() {}
    ...
    */
    ...
    GetMethodsMap(): Map<string, ArkMethod> {
        let res : Map<string, ArkMethod> = new Map();
        for (const cls of this.getClassesMap().values()) {
            for (const method of cls.getMethods(true)) {
                res.set(method.getSignature().toString(), method);
            }
        }
        return res;
    }
