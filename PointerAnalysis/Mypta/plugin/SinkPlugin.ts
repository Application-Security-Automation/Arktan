    ...
    FindTaintFlows(cs: CallSite, sink: Sink) : TaintFlow[] {
        ...
        this.solver.ptrManager.getCSVar(arg).getObjects().forEach(obj => {
            if(obj instanceof TaintObj) {
                ...
                let len = Number(obj.getNewArrayExpr().getSize() as Constant) 
                for(let i=0; i<len; i++) {
                    this.solver.ptrManager.getArrayIndex(obj,String(i)).getObjects().forEach(o => {
                        if(o instanceof TaintObj) {
                            tfres.push(new TaintFlow(o.getSourcePoint(),sinkpoint));
                        }
                    })
                }
            }
